...
const mobileTheme = extendTheme({
  styles: { global: { body: { fontSize: 'sm' } } },
  breakpoints: { base: '0em', sm: '30em', md: '48em', lg: '62em', xl: '80em' },
});
  ...
  if (process.env.NEXT_PUBLIC_RUM_ENDPOINT) {
    try {
      navigator.sendBeacon?.(
        ...
        JSON.stringify({ app: 'mobile-optimized', metric })
      );
    } catch {}
  }
}
...
function GlobalStateBridge() {
  ...
  useEffect(() => {
    ...
    mod.useRegisterInitialState({ 'mfe.mobile.ready': true });
  }, [mod]);
  return null;
}
